    ...
    <div class="data-content-wrapper">
      <el-form :model="ruleForm" :rules="rules" :label-position="labelPosition" ref="ruleForm" v-loading.fullscreen.lock="loadings" element-loading-text="拼命加载中">
        ...
        <el-form-item label="应用包名：" prop="packages">
          <el-input v-model="ruleForm.packages" placeholder="请输入应用包名"></el-input>
        </el-form-item>
...
export default {
  ...
  data () {
    ...
    return {
      ...
      breadContent: [{ text: '应用管理', path: '/dev/app/index'}, { text: '修改应用'}],
      industryCategories1: [],
      industryCategories2: [],
      industryCategory1:'',
	    industryCategory2:'',
  ...
  methods: {
    ...
    getslots () {
      this.loadings = true;
      this.$http.get('/v1/dev/apps/'+this.appId+'/adslots').then((res) => {
        this.loadings = false;
        let data = res.body;
        if (data.ret!=1) {
          return this.$alert(data.message, '提示：', {
    ...
    creates () {
      ...
      this.$http.put('/v1/dev/apps/'+this.appId, params).then((res) => {
        this.loadings = false;
        let data = res.body;
        if (data.ret!=1) {
          return this.$alert(data.message, '提示：', {
